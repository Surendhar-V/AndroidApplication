...
//        toast.show()
        ...
        binding.edEq.showSoftInputOnFocus = false
    ...
    private fun changeTheme(){
        ...
        if(isNightMode){
            ...
            // Changing the app into light mode
            isNightMode = false
            binding.btnTheme.setImageResource(R.drawable.blue_bulb)
            AppCompatDelegate.setDefaultNightMode(AppCompatDelegate.MODE_NIGHT_NO)
        }else{
            // The app is in dark mode
            // Changing the app into dark mode
            isNightMode = true
            binding.btnTheme.setImageResource(R.drawable.gray_bulb)
    ...
    private fun equal(){
        ...
        var open_bracket_count = isBalanced(binding.edEq.text.toString() , binding.edEq.selectionStart)

        var s : String = binding.edEq.text.toString()
        if( s.length!=0 &&  ( (s.get(s.length-1).equals(getString(R.string.addText)))
            || (s.get(s.length-1).equals(getString(R.string.subtractText)))
            || (s.get(s.length-1).equals(getString(R.string.multiplyText)))
            || (s.get(s.length-1).equals(getString(R.string.divideText)))
            || (s.get(s.length-1).equals(getString(R.string.moduloText)))
                    )){
...
//        TODO("Long press on back space")
        ...
        if(cursorpos != 0 && binding.edEq.text.length != 0) {
            ...
            toast.cancel()
            toast = Toast.makeText(applicationContext , "Empty Equation detected" , Toast.LENGTH_SHORT)
            toast.show()
        }
    ...
    private fun bracket(){
        ...
        var close_bracket_bool : Boolean = isBalanced(binding.edEq.text.toString() , binding.edEq.selectionStart) != 0

        /*

        Separate into two sections

            1 - when the selection is in between

            2 - when the selection is at the end

                Make use of two variables

                    1 - Open_bracket_count
                    2 - close_bracket_bool

                    Open_bracket_count will have the track of how many brackets are needed to be closed
                    close_bracket will indicate , when the close bracket can be used

                    fTODO("") . => .0*(
                    TfODO("") brackets will not be entered at the initial position
                    TfODO("") lastString != divide , multiply , add , subtract , modulo , decimal then close_bracket can be activated

                    If close_bracket is true
                    fTODO("") update the text with the close bracket when clicked
                    possible cases:

                                    close_bracket is true and at the selection there is an operator
                                    close_bracket is true and at the selection there is an number including .
                                    close_bracket is true and at the selection there is another close bracket
                                    close_bracket is true and at the selection there is a open bracket

                    Is close_bracket is false
                    fTODO("") update the text with the open bracket when clicked
                    possible cases:

                                close_bracket is false and at the selection there is an operator
                                close_bracket is false and at the selection there is an number including .
                                close_bracket is false and at the selection there is another close bracket
                                close_bracket is false and at the selection there is a open bracket
        ...
        if(binding.edEq.selectionStart == 0){
            ...
            lastString = binding.edEq.text.toString().get(binding.edEq.selectionStart-1).toString()
        }
        ...
        if(binding.edEq.selectionStart != binding.edEq.text.length){
            ...
            Log.i(TAG, "bracket: stage 13 crossed")
                if(close_bracket_bool){
                    ...
                    // If lastString == digit
                    if((!lastString.equals(getString(R.string.multiplyText))
                        && !lastString.equals(getString(R.string.divideText))
                        && !lastString.equals(getString(R.string.addText))
                        && !lastString.equals(getString(R.string.subtractText))
                        && !lastString.equals(getString(R.string.moduloText))
                        && !lastString.equals(getString(R.string.decimalText))
                        && !lastString.equals(getString(R.string.open_parentheses)))
                        || lastString.equals(getString(R.string.closed_parentheses))
                      ){
                        Log.i(TAG, "bracket: stage 15 crossed")
                        updateText(getString(R.string.closed_parentheses))
                        var string  = binding.edEq.text.toString().get(binding.edEq.selectionStart).toString()
                        Log.i(TAG, "bracket: right String => ${string}")
                        if(!string.equals(getString(R.string.addText))
                            || !string.equals(getString(R.string.subtractText))
                            || !string.equals(getString(R.string.multiplyText))
                            || !string.equals(getString(R.string.divideText))
                            || !string.equals(getString(R.string.moduloText))
    ...
    if(!close_bracket_bool && ( lastString.equals(getString(R.string.closed_parentheses))
                        || lastString.equals(getString(R.string.decimalText))
                        || (   !lastString.equals(getString(R.string.addText))
                        && !lastString.equals(getString(R.string.subtractText))
                        && !lastString.equals(getString(R.string.multiplyText))
                        && !lastString.equals(getString(R.string.divideText))
                        && !lastString.equals(getString(R.string.moduloText))
                        ) ) ){
            ...
            updateText(getString(R.string.open_parentheses))
                ...
                if(lastString.equals(getString(R.string.addText))
                    || lastString.equals(getString(R.string.subtractText))
                    || lastString.equals(getString(R.string.multiplyText))
                    || lastString.equals(getString(R.string.divideText))
                    || lastString.equals(getString(R.string.moduloText))){
...
//       TODO("No Dot in the same number set")
        ...
        if( binding.edEq.text.length != 0 &&
                    lastString.equals(getString(R.string.open_parentheses)) && (
                    strToAdd.equals(getString(R.string.divideText))
                ||  strToAdd.equals(getString(R.string.multiplyText))
                ||  strToAdd.equals(getString(R.string.addText))
                ||  strToAdd.equals(getString(R.string.subtractText))
        ...
        // +. => +0.
        if(strToAdd.equals(getString(R.string.decimalText))  && (
                lastString.equals(getString(R.string.addText))
            ||  lastString.equals(getString(R.string.subtractText))
            ||  lastString.equals(getString(R.string.divideText))
            ||  lastString.equals(getString(R.string.moduloText))
        ...
        //  .+ => .0+
        if(lastString.equals(getString(R.string.decimalText)) && (  strToAdd.equals(getString(R.string.multiplyText))
                    || strToAdd.equals(getString(R.string.moduloText))
                    || strToAdd.equals(getString(R.string.divideText))
                    || strToAdd.equals(getString(R.string.subtractText))
                    || strToAdd.equals(getString(R.string.addText))
        ...
        if(  strToAdd.equals(getString(R.string.divideText))
                    || strToAdd.equals(getString(R.string.multiplyText))
                    || strToAdd.equals(getString(R.string.addText))
                    || strToAdd.equals(getString(R.string.subtractText))
                    || strToAdd.equals(getString(R.string.moduloText))
                    || strToAdd.equals(getString(R.string.closed_parentheses))
